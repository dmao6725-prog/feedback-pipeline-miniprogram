...
- [ ] 首页标题和副标题完整显示，没有被顶部导航挤压
- [ ] “运行分析”主按钮位于运行参数卡片底部，不在标题右侧
- [ ] 输入框 placeholder 没有被裁切，文字垂直居中
- [ ] 选择文件后文件列表出现，文件名、大小、类型和删除按钮可见
- [ ] 无文件时主按钮显示“请先选择文件”，选择有效文件后变为可点击
- [ ] noLLM 开启时按钮文案为“开始清洗分析”，AI 模式为“开始 AI 分析”
- [ ] 历史页空状态有“开始第一次分析”按钮，并能跳转到分析页
- [ ] 设置页内容没有被 tabBar 遮挡
- [ ] Android 模拟器和 iOS 模拟器均显示正常

## 图标与路由修复验证

1. 底部 tabBar 是否显示三个本地 PNG 图标。
2. 选中 tab 时图标是否变蓝。
3. 历史页空状态是否显示剪贴板/文档图标，而不是字母 H。
4. 首页上传区是否显示本地上传文件图标。
5. 点击“开始第一次分析”是否能切回分析 tab。
6. 云环境未配置时，点击分析是否显示明确提示，而不是 timeout。

## 云环境未配置时的验证

保持 `miniprogram/app.js` 中 `CLOUD_ENV_ID` 为 `your-env-id`，重新编译小程序。

**验证点**：
- [ ] 启动后不应出现红色 `Error: timeout`
- [ ] 控制台只允许出现 `[App] 请在 miniprogram/app.js 中配置真实云环境 ID`、`getSystemInfo`、基础库灰度等黄色提示
- [ ] 分析页、历史页、设置页初始化时不应调用云函数
- [ ] 直接进入无 `taskId` 的结果页时显示空状态，不调用 `getTaskResult`
- [ ] 点击“开始分析”时弹窗提示需要配置云环境，不进入文件上传、云函数调用或超时等待
